import sys
sys.path.append("C:\\Users\\Shanty\\Desktop\\shanty\\Skripsi_KP\\Deep em classifier\\sp_tool")
...
def run(args):
    ...
    model = keras.models.load_model(args.model_path,
                                    custom_objects={'macro_F1_score': blstm_model_val.macro_F1_score,
                                                    'f1_SP': blstm_model.f1_SP,
    ...
    for original_obj, out_fname, predicted_labels in zip_equal(arff_objects, out_fnames, predictions['pred']):
        ...
        ##################################################################
        # for fixation <= 0.95 and saccade <= 0.88 set to UNKNOWN (class : 0)
        max_pred = np.max(predicted_labels, axis=-1)
        max_index = np.argmax(predicted_labels, axis=-1)
        # print(max_pred[:8])
        # print("eeeeeeeeeeeeeeeeeennnddd")
        # print(max_index[:8])
        # print("eeeennnnnnnnnnnnnnde2")
        # mask_fix = (max_index == 1) & (max_pred >= 0.65)
        # mask_sacc = (max_index == 2) & (max_pred >= 0.65)
        # mask_noise = (max_index == 4) & (max_pred >= 0.65)
        # mask_sp = (max_index == 3) & (max_pred >= 0.5) 
        # mask_other = (max_index == 0) | (max_index == 3)
        mask_other = (max_index == 0) | (max_index == 3) | (max_index == 1) | (max_index == 2) | (max_index == 4)
        mask = mask_other
        # mask = mask_fix | mask_sacc | mask_noise | mask_other 
        zeros = np.zeros((predicted_labels.shape[0], predicted_labels.shape[1]))
        labels_pred = np.where(mask, max_index, zeros)
        ####################################################################
        ...
        # Fill in with numerical labels
        original_obj['data'][sp_processor.EM_TYPE_ATTRIBUTE_NAME] = labels_pred
