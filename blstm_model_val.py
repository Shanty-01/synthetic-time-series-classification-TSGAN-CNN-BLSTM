...
def macro_F1_score(y_true, y_pred, dtype=None):
    f1_FIX = categorical_f1_score_for_class(y_true, y_pred, 1, dtype)
    f1_SACC = categorical_f1_score_for_class(y_true, y_pred, 2, dtype)
    f1_SP = categorical_f1_score_for_class(y_true, y_pred, 3, dtype)
    f1_noise = categorical_f1_score_for_class(y_true, y_pred, 4, dtype)
    macro_F1 = (f1_FIX + f1_SACC + f1_SP+ f1_noise)/4
    return macro_F1
...
##########################################################################################
def get_extracted_feature(model, X, y=None,
    ...
    res_X = np.pad(res_X,
                   ...
                   mode='constant')
    print(f'input to model num_samples is multiple of batch size: {res_X.shape}')
    # take only the needed predictions
    res_proba = model.predict(res_X, batch_size=batch_size)
...
def run(args):
    ...
    files_template_val = args.feature_files_folder_val + '/{}/*.arff'
    ...
    gc_val = json.load(open('data/inputs/GazeCom_video_parameters_val.json'))
    all_video_names = gc['video_names']
    all_video_names_val = gc_val['video_names']
    ...
    if not os.path.exists(raw_data_set_fname):
        ...
        data_Y_one_hot_val = []
        total_files_val = 0
        for video_name in gc_val['video_names']:
            print('For {} using files from {}'.format(video_name, files_template_val.format(video_name)))
            fnames = sorted(glob.glob(files_template_val.format(video_name)))
            total_files_val += len(fnames)

            data_X_val.append([])
            data_Y_val.append([])
            data_Y_one_hot_val.append([])
            if args.final_run:
                source_fnames.append(list(fnames))
                source_objs.append([])

            for f in fnames:
                o_val = ArffHelper.load(open(f))

                # normalize coordinates in @o by dividing by @ppd_f -- the pixels-per-degree value of this file @f
                ppd_f = sp_util.calculate_ppd(o_val)
                for k in keys_to_convert_to_degrees:
                    o_val['data'][k] /= ppd_f

                # add to respective data sets (only the features to be used and the true labels)
                data_X_val[-1].append(np.hstack([np.reshape(o_val['data'][key], (-1, 1)) for key in keys_to_keep]).astype(np.float64))
                assert data_X_val[-1][-1].dtype == np.float64
                if 'time' in keys_to_keep:
                    data_X_val[-1][-1][:, keys_to_keep.index('time')] *= time_scale
                data_Y_val[-1].append(o_val['data']['handlabeller_final'])  # "true" labels
                data_Y_one_hot_val[-1].append(np.eye(num_classes)[data_Y_val[-1][-1]])  # convert numeric labels to one-hot form
        ...
        if total_files_val > 0:
            print('Loaded a total of {} val files'.format(total_files_val))
        else:
            raise ValueError('No input files found! Check that the directory "{}" exists '
                             'and is accessible for reading, or provide a different value for the '
                             '--feature-files-folder argument. Make sure you extracted the respective archive, '
                             'if the data was provided in this way. This folder must contain '
                             '18 subfolders with names corresponding to '
                             'GazeCom clip names.'.format(args.feature_files_folder_val))
    ...
    for subset_index in range(len(data_X_val)):
        x_val, y_val = extract_windows(data_X_val[subset_index], data_Y_one_hot_val[subset_index],
                               window_length=args.window_size,
                               downsample=downsample,
                               padding_features=padding_features)
        windows_X_val.append(np.array(x_val))
        windows_Y_val.append(np.array(y_val))

        # Subtract the first value of each feature inside each window
        for col_ind in keys_to_subtract_start_indices:
            windows_X_val[-1][:, :, col_ind] -= windows_X_val[-1][:, 0, col_ind].reshape(-1, 1)
    ...
    for i, video_name in enumerate(all_video_names):
        ...
        if not ALREADY_TRAINED:
            ...
            r = np.random.RandomState(0)
            # here, we ignore the "left out" video clip @i, but aggregate over all others
            train_set_len_val = sum([len(windows_X_val[j]) for j in range(len(windows_X_val))])
            print('Total amount of windows:', train_set_len_val)
            # will shuffle all samples according to this permutation, and will keep only the necessary amount
            # (by default, 50,000 for training)
            perm = r.permutation(train_set_len_val)[:args.num_training_samples_val]
            train_X_val = []
            train_Y_val = []
            indices_range_low = 0
            for j in range(len(windows_X_val)):
                # if j == i:
                #     continue  # skip the test set
                indices_range_high = indices_range_low + len(windows_X_val[j])
                # permutation indices [indices_range_low; indices_range_high) are referring
                # to the windows in @windows_X[j]
                local_indices = perm[(perm >= indices_range_low) * (perm < indices_range_high)]
                # re-map in "global" indices (inside @perm) onto "local" indices (inside @windows_X[j])
                local_indices -= indices_range_low
                train_X_val.append(windows_X_val[j][local_indices])
                train_Y_val.append(windows_Y_val[j][local_indices])

                indices_range_low = indices_range_high

            # assemble the entire val set
            train_X_val = np.concatenate(train_X_val)
            train_Y_val = np.concatenate(train_Y_val)
            ...
            if args.initial_epoch:
                ...
                model = keras.models.load_model(MODEL_PATH + '/Conv_sample_windows_epochs_{}_without_{}.h5'.format(args.initial_epoch, video_name),
                                                custom_objects={'macro_F1_score': macro_F1_score, 'f1_SP': f1_SP, 'f1_SACC': f1_SACC, 'f1_noise': f1_noise,'f1_FIX': f1_FIX})
                ...
                # plot_model(model, to_file='run4/model_run4.png', show_shapes=True, show_layer_names=True)
        else:
            model = keras.models.load_model(model_fname, custom_objects={'macro_F1_score' : macro_F1_score,
                                                                         'f1_SP': f1_SP,
                                                                         'f1_SACC': f1_SACC,
                                                                         'f1_noise': f1_noise,
                                                                         'f1_FIX': f1_FIX})
        ...
        if TRAINING_MODE and not ALREADY_TRAINED:
            ...
            assert train_X.shape[0] == args.num_training_samples, 'Not enough training samples (might need to ' \
                                                                  ...
                                                                  'line arguments.'
            checkpoint_filepath = model_fname
            ...
            training_histories.append(model.fit(train_X, train_Y,
                                    ...
                                    #   validation_split= 0.032258064516129,
            ...
            history = training_histories[0]
            # Get the training and validation metrics
            training_accuracy = history.history['accuracy']
            training_macro_F1 = history.history['macro_F1_score']
            training_f1_SP = history.history['f1_SP']
            training_f1_FIX = history.history['f1_FIX']
            training_f1_SACC = history.history['f1_SACC']
            training_f1_noise = history.history['f1_noise']
            validation_loss = history.history['val_loss']
            validation_accuracy = history.history['val_accuracy']
            validation_macro_F1 = history.history['val_macro_F1_score']
            validation_f1_SP = history.history['val_f1_SP']
            validation_f1_FIX = history.history['val_f1_FIX']
            validation_f1_SACC = history.history['val_f1_SACC']
            validation_f1_noise = history.history['val_f1_noise']
            np_validation_macro_F1 = np.array(validation_macro_F1)
            np_validation_fix = np.array(validation_f1_FIX)
            np_validation_sp = np.array(validation_f1_SP)
            np_validation_sacc = np.array(validation_f1_SACC)
            np_validation_noise = np.array(validation_f1_noise)

            print(f'\n Best validation macro F1 : {np.max(np_validation_macro_F1)}\n at epoch : {np.argmax(np_validation_macro_F1)+1}\n')
            print(f'\n Best validation F1 fixation : {np.max(np_validation_fix)}\n at epoch : {np.argmax(np_validation_fix)+1}\n')
            print(f'\n Best validation F1 sp : {np.max(np_validation_sp)}\n at epoch : {np.argmax(np_validation_sp)+1}\n')
            print(f'\n Best validation F1 saccade : {np.max(np_validation_sacc)}\n at epoch : {np.argmax(np_validation_sacc)+1}\n')

            # Plot training and validation loss
            epochs = range(1, len(training_accuracy) + 1)
            plt.figure(figsize=(20, 20))
            # Plot training and validation macro_F1_score
            plt.subplot(4, 1, 1)
            plt.plot(epochs, training_macro_F1, 'b', label='Training macro F1 score')
            plt.plot(epochs, validation_macro_F1, 'r', label='Validation macro F1 score')
            plt.title('Macro F1 Score in Training and Validation', fontsize=18)
            # plt.title('Macro F1 Score in Training', fontsize=18)
            ...
            plt.subplot(4, 1, 2)
            plt.plot(epochs, training_f1_SP, 'b', label='Training F1 SP')
            plt.plot(epochs, validation_f1_SP, 'r', label='Validation F1 SP')
            plt.title('SP F1 Score in Training and Validation', fontsize=18)
            # plt.title('SP F1 Score in Training ', fontsize=18)
            plt.xlabel('Epochs', fontsize=16)
            plt.ylabel('F1 SP Score', fontsize=16)
            plt.legend(fontsize=14)
            plt.ylim(0, 1)

            plt.subplot(4, 1, 3)
            plt.plot(epochs, training_f1_FIX, 'b', label='Training F1 Fixation')
            plt.plot(epochs, validation_f1_FIX, 'r', label='Validation F1 Fixation')
            # plt.title('Fixation F1 Score in Training', fontsize=18)
            plt.title('Fixation F1 Score in Training and Validation', fontsize=18)
            plt.xlabel('Epochs', fontsize=16)
            plt.ylabel('F1 Fixation Score', fontsize=16)
            plt.legend(fontsize=14)
            plt.ylim(0, 1)

            plt.subplot(4, 1, 4)
            plt.plot(epochs, training_f1_SACC, 'b', label='Training F1 Saccade')
            plt.plot(epochs, validation_f1_SACC, 'r', label='Validation F1 Saccade')
            plt.title('Saccade F1 Score in Training and Validation', fontsize=18)
            # plt.title('Saccade F1 Score in Training', fontsize=18)
            plt.xlabel('Epochs', fontsize=16)
            plt.ylabel('F1 Saccade Score', fontsize=16)
            plt.legend(fontsize=14)
            plt.ylim(0, 1)
        ...
        if args.fid:
            output_layer_name = 'bidirectional'
            output_layer_index = model.layers.index(model.get_layer(output_layer_name))
            new_model = keras.Model(inputs=model.input, outputs=model.layers[output_layer_index].output)
            for layer in new_model.layers :
                layer.trainable = False 
            new_model.summary()
            new_model.save(model_fname)
...
def parse_args(dry_run=False):
    ...
    parser.add_argument('--feature-files-folder-val', '--feature-folder-val', '--feat-folder-val',
                        default='data/inputs/GazeCom_all_features/',
                        help='Folder containing the file for validation.')
    parser.add_argument('--fig-name', default='figname.png', help='fig name')
    parser.add_argument('--fid', default=False, help='whether to make an fid model')
    ...
    parser.add_argument('--training-samples-val', dest='num_training_samples_val', default=50000, type=int,
                        help='Number of validation samples. The default value is appropriate for windows of 65 samples, '
                             'no overlap between the windows. If window size is increased, need to set --overlap to '
                             'something greater than zero, ideally - maintain a similar number of windows. For example,'
                             ' if we use windows of size 257, we set overlap to (257-65) = 192, so that there would be '
                             'as many windows as with a window size of 65, but without overlap.\n\n'
