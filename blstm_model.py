...
def create_model(num_classes, batch_size, train_data_shape, dropout_rate=0,
                 padding_mode='valid',
...
def run(args):
    ...
    for i, video_name in enumerate(all_video_names):
        ...
        if TRAINING_MODE and not ALREADY_TRAINED:
            ...
            plt.title('SP F1 Score in Training and Validation')
            plt.xlabel('Epochs')
            plt.ylabel('Accuracy')
            plt.legend()

            plt.subplot(4, 1, 3)
            plt.plot(epochs, training_f1_FIX, 'b', label='Training F1 FIX')
            plt.plot(epochs, validation_f1_FIX, 'r', label='Validation F1 FIX')
            plt.title('FIX F1 Score in Training and Validation')
            plt.xlabel('Epochs')
            plt.ylabel('Accuracy')
            plt.legend()

            plt.subplot(4, 1, 4)
            plt.plot(epochs, training_f1_SACC, 'b', label='Training F1 SACC')
            plt.plot(epochs, validation_f1_SACC, 'r', label='Validation F1 SACC')
            plt.title('SACC F1 Score in Training and Validation')
            plt.xlabel('Epochs')
            plt.ylabel('Accuracy')
            plt.legend()
            ...
            plt.tight_layout()
            fig_name = 'run7_brd6files/figure1500.png' 
